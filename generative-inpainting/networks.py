...
class FineGenerator(nn.Module):
    def __init__(self, input_dim, cnum, use_cuda=True, device_ids=None):
        ...
        # self.NH = 8
        # self.MH = nn.Parameter(torch.empty(self.NH, 1, cnum * 8, 1, 1))
    ...
    def forward(self, xin, x_stage1, mask):
        ...
        # N, C, H, W = x.size()
        # x.unsqueeze(0)
        # out = x.repeat(self.NH, 1, 1, 1, 1)
        # out = out * torch.sigmoid(self.MH)
        # x = out.view(N * self.NH, C, H, W)
...
class ContextualAttention(nn.Module):
    ...
    def forward(self, f, b, mask=None):
        ...
        for idx, (xi, wi, raw_wi) in enumerate(zip(f_groups, w_groups, raw_w_groups)):
            '''
            ...
            # softmax to match
            mm = (reduce_mean(m[idx], axis=[1, 2, 3], keepdim=True) == 0.).to(torch.float32)
            mm = mm.permute(1, 0, 2, 3) # mm shape: [1, L, 1, 1]
            yi = yi * mm
...
class LocalDis(nn.Module):
    def __init__(self, config, use_cuda=True, device_ids=None):
        ...
        self.linear = nn.Linear(self.cnum*8*4*2, 1)
...
class GlobalDis(nn.Module):
    def __init__(self, config, use_cuda=True, device_ids=None):
        ...
        self.dis_conv_module = DisConvModule(self.input_dim, self.cnum)
        self.linear = nn.Linear(self.cnum*8*8*4, 1)
        # self.clss_linear = nn.Linear(self.cnum*8*8*4 ,179)

    def forward(self, x):
        x = self.dis_conv_module(x)
        x = x.view(x.size()[0], -1)
        x_1 = self.linear(x)
        # x_2 = self.clss_linear(x)
        # return x_1, x_2
        return x_1
